...
function AddToWishlist( appid, divToHide, divToShowSuccess, divToShowError, navref, divToHide2 )
{
	if ( !g_AccountID )
	{
		ShowAlertDialog( "Effettua l'accesso...", "Devi accedere a Steam per aggiungere un articolo alla tua lista dei desideri." )
		.done( function() {
		...
		$J.get( url, {ll: 'italian'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'https://store.steampowered.com/tag/it/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Errore', 'Si è verificato un problema durante il tentativo di seguire il curatore di Steam. Possibili cause:<ul><li>Non puoi seguire un curatore che stai ignorando.</li><li>Puoi seguire un massimo di 100 curatori alla volta</li><li>Gli account limitati non possono seguire curatori</li></ul>' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Errore', 'Si è verificato un problema durante il tentativo di ignorare il curatore di Steam.<br>Nota: non puoi ignorare un curatore che segui.' );
		}
	);
	return false;
}
