...
function FillFormFromNavigation( querystring, link_click, initial_load )
{
	...
	if ( rgLocationParams['term'] )
		...
		rgLocationParams['displayterm'] = 'Inserisci il termine di ricerca o l\'etichetta';
...
function UpdateTags()
{
	...
	if( strPublisher )
	{
		AddSearchTag( 'publisher', strPublisher, "Editore" + ': '+strPublisher, function(tag) { $J("#publisher").val(""); AjaxSearchResults(); return false; } );
		$J('#termsnone').hide();
	}
	...
	if( strDeveloper )
	{
		AddSearchTag( 'publisher', strDeveloper, "Sviluppatore" + ': '+strDeveloper, function(tag) { $J("#developer").val(""); AjaxSearchResults(); return false; } );
		$J('#termsnone').hide();
	}
...
function OnSelectFilteredContentSettingsMenu( elSource )
{
	...
	{
		case 'explain':
			ShowAlertDialog( "Filtro dei contenuti", "Le preferenze impostate determinano i titoli che ti vengono mostrati nel Negozio di Steam, inclusa la pagina iniziale, i risultati di ricerca ecc." );
			break;
		case 'preferences':
